...
export async function sendCompanyVerificationEmail(input: {
  ...
  const subject =
    input.locale === "hr"
      ? `Potvrda identiteta tvrtke — ${headerCompanyName}`
      : `Confirm your company identity — ${headerCompanyName}`;
  ...
  const bodyHr = `
    ...
    <p><a href="${safeConfirmUrl}" style="display:inline-block;background:#10b981;color:#fff;padding:12px 22px;border-radius:9999px;text-decoration:none;font-weight:600">Potvrdi tvrtku</a></p>
  ...
  const bodyEn = `
    ...
    <p><a href="${safeConfirmUrl}" style="display:inline-block;background:#10b981;color:#fff;padding:12px 22px;border-radius:9999px;text-decoration:none;font-weight:600">Confirm company</a></p>
