...
export function CompanyExportsSection({ companyId, memberRole, subscriptionTier }: Props) {
  ...
  const [framework, setFramework] = useState<Framework | "">(
    (allowedFrameworks[0] as Framework | undefined) ?? ""
  );
  ...
  const load = useCallback(async () => {
    if (!canFinance || !flags.exportsEnabled) {
      setLoading(false);
      return;
    }
    try {
  ...
  const tierAllows = allowedFrameworks.length > 0;
  ...
  return (
    <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
      ...
      {!flags.exportsEnabled ? (
        <p className="text-sm text-amber-700 dark:text-amber-400">{t("export.feature_off")}</p>
      ) : !tierAllows ? (
        <p className="text-sm text-amber-700 dark:text-amber-400">{t("export.tier_required")}</p>
      ) : loading ? (
        ...
        <>
          <div className="mb-4 flex flex-wrap items-end gap-3">
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
              ...
              >
                {allowedFrameworks.map((f) => (
                  <option key={f} value={f}>
                    {locale === "hr" ? FRAMEWORK_LABELS[f as Framework].labelHr : FRAMEWORK_LABELS[f as Framework].label}
                  </option>
                ))}
              </select>
            </label>
