...
export function CompanyReceiptsSection({ companyId, memberRole, subscriptionTier }: Props) {
  ...
  const load = useCallback(async () => {
    if (!canFinance || !flags.receiptsEnabled) {
      setLoading(false);
      return;
    }
    try {
  ...
  return (
    <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
      ...
      {!flags.receiptsEnabled ? (
        <p className="text-sm text-amber-700 dark:text-amber-400">{t("receipt.feature_off")}</p>
      ) : !tierAllows ? (
        <p className="text-sm text-amber-700 dark:text-amber-400">{t("receipt.tier_required")}</p>
      ) : loading ? (
