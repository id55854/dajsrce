...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [needs, events] = await Promise.all([
      ...
      supabase.from("volunteer_events").select("id, title, description, event_date, start_time, end_time, requirements, location, volunteers_needed, volunteers_signed_up, created_at").eq("institution_id", profile.institution_id).order("created_at", { ascending: false }).limit(LIMIT + 1),
    ]);
