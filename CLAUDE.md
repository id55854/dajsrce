...
39. `20260922120000_map_multi_donation_types.sql` (map donation-type filter is multi-select; drops and recreates `map_association_registry_v1`/`v2` with a trailing `p_donation_types text[]`). **Not applied on Neon** (verified 2026-09-26): two or more donation types run on the bounded fallback. Never replay this file, since it predates 47 and would drop the curated institutions from the map; add `p_donation_types` in a new migration built on the 47 body.
...
49. `20260926100000_close_need_event_update_hole.sql` (applied 2026-09-26: drops the hand-made `USING (true)` UPDATE policies on `needs`/`volunteer_events`, revokes UPDATE on both from `anon`/`authenticated`, and revokes UPDATE (name, category) on `institutions`. Any signed-in account could previously PATCH any organisation's need or event through the Data API.)
50. `20260926110000_launch_claims_editing_and_safety.sql` (applied 2026-09-26). Claims: approval adopts an unheld rule-promoter institution (`source = 'registry'`, no profile, no approved claim; `institution_is_adoptable`) instead of refusing it; `search_claimable_associations_v1` folds diacritics/punctuation with `hr_fold`, requires every term, ranks exact OIB/UDR_ID/registry number first and reports `truncated`; approval requires the register mailbox challenge **or** a review note starting `Provjereno:`; the challenge always goes to the register address; request/approve/reject notify admins/applicants in-app; `list_institution_claims_for_review` returns `total` and works the open queue oldest first; `complete_profile_setup('individual')` returns an unlinked `ngo` without an open claim to `individual`. Editing: direct authenticated UPDATE on `institutions` is revoked; organisations edit only through the service-role RPCs `update_own_institution_profile`, `update_need_transaction` and `update_volunteer_event_transaction` (ownership decided in the RPC; event date/time/location changes notify signed-up volunteers). Abuse: triggers cap each account at 10 new pledges and 10 new signups per 24 h and 25 active of each. Reminders use the Europe/Zagreb date; `send_volunteer_event_reminders_with_recipients()` returns only newly reminded recipients so the cron can e-mail each once. Also the role-checked NGO pledge read policy, text length CHECKs, notification policy cleanup and TRUNCATE/REFERENCES/TRIGGER revokes.
51. `20260926111000_claim_search_fold_index.sql` (applied 2026-09-26; `CREATE INDEX CONCURRENTLY` on `hr_fold(search_text)`, so apply it on its own, outside a transaction).
52. `20260926120000_profiles_institution_requires_ngo.sql` (applied 2026-09-26; idempotent: unlinks non-`ngo` profiles from institutions and adds `profiles_institution_requires_ngo`).
