-- A bounded public listing of the organisations that actually have an account
-- here, for the merged "Associations" page.
--
-- The register view answers "does this association exist officially" over
-- 43,703 rows. It cannot answer "which of these can I actually help right
-- now", because being in the register says nothing about having an account or
-- an open need -- and conflating the two is exactly what the project's rules
-- forbid. So this is a separate, much smaller query over the subset that is
-- linked to an institution, optionally narrowed to those with an open need.
--
-- The existing `search_association_registry_v1` is deliberately left alone: it
-- is the register's own contract, and adding optional parameters to it would
-- make the eight-argument call ambiguous during a rolling deploy.

BEGIN;

-- The subset is small and the join is by institution, so this index is what
-- keeps the listing off a full scan of the published projection.
CREATE INDEX IF NOT EXISTS idx_registry_directory_entries_engaged
  ON public.registry_directory_entries (batch_id, name COLLATE public.hr_sort, udr_id)
  WHERE institution_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_needs_open_institution
...
REVOKE ALL ON FUNCTION public.engaged_association_directory_v1(
  text, text, text, boolean, boolean, integer, integer
) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.engaged_association_directory_v1(
  text, text, text, boolean, boolean, integer, integer
