-- Fix: the place-clustering function could not run.
--
-- 20260822160000 added place_kind and place_name to the RETURNS TABLE of
-- map_association_registry_v1. In a plpgsql function every RETURNS TABLE column
-- is also a variable in scope for the whole body, so the CTEs that produced
-- columns of the same name made every unqualified reference to them ambiguous:
--
--   ERROR 42702: column reference "place_kind" is ambiguous
--   DETAIL: It could refer to either a PL/pgSQL variable or a table column.
--
-- That is a parse-time failure of the RETURN QUERY, so it fired on every call
-- and the public map returned 503 for every viewport. It survived review
-- because the migration was rehearsed by applying it and running the loader,
-- neither of which calls the function - only executing the query reaches the
-- ambiguity.
--
-- The internal grouping columns are renamed grp_kind/grp_name and every
-- reference is qualified; the two output columns keep their names and meaning,
-- so no caller changes. Nothing else in the body differs from 20260822160000.
