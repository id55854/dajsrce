-- Curated institutions on the map.
--
-- The map reads the associations register (registry_directory_entries), so
-- the 38 reviewed institutions that are not registered associations (Caritas
-- services, parish and city soup kitchens, state children's homes, Red Cross
-- and Caritas shelters) never appeared, even though their detail pages work.
-- They are now a second candidate source with the same filters, clustering and
-- truncation. Only source = 'curated' AND is_verified rows that no registry row
-- links to are added; hidden locations contribute their coarse public point
-- only (invariant 1). Detail stays public_institution_detail_v1.
  ...
  WITH urgent_institutions AS MATERIALIZED (
    ...
    WHERE i.source = 'curated'
      ...
      AND (
        query_terms IS NULL
        OR (
          coalesce(i.search_text, lower(i.name)) ILIKE '%' || query_terms[1] || '%'
          AND NOT EXISTS (
            SELECT 1 FROM unnest(query_terms[2:]) AS term
            WHERE coalesce(i.search_text, lower(i.name)) NOT ILIKE '%' || term || '%'
          )
        )
      )
      AND (
...
-- The county of a curated institution is read from any register row in the
-- same city; this keeps that lookup an index probe.
CREATE INDEX IF NOT EXISTS idx_registry_directory_entries_batch_city
  ON public.registry_directory_entries (batch_id, city)
