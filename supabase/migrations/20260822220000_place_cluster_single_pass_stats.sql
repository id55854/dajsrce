-- Collapse the tier statistics into one pass.
--
-- 20260822200000 needed a group count and a largest-group size per tier, and
-- got them from four separate GROUP BY passes over the materialised candidate
-- set. Correct, and too slow: the Zagreb zoom-12 tail went from 1.46s to 3.68s
-- over eight samples, which is back across the 3s statement_timeout on the anon
-- role - the exact failure the storage and query work earlier in this sequence
-- was undertaken to remove.
--
-- Postgres can compute all four in a single hash aggregate over GROUPING SETS,
-- and the match count falls out of the county set for free because key_county
-- is never null. The tier decision and every other part of the body are
-- unchanged; this is purely how the numbers feeding it are gathered.
