-- Choose the clustering tier by whether it divides the viewport, not only by
-- whether it fits.
--
-- 20260822160000 picked the finest named tier whose group count landed between
-- 2 and the feature budget. Two viewports showed that is not enough:
--
--   Dalmatia at z9   3 counties, one holding 90% of the rows. Every town on
--                    the coast collapsed into "Splitsko-dalmatinska - 3,829".
--   Donji grad z15   2 districts, one holding 85%. Zoomed into a few blocks,
--                    the map answered with the district already filling the
--                    screen instead of the streets inside it.
--
-- Both are the same failure: the finer tier overshot the budget by a little,
-- and the rule fell all the way through to a tier coarser than the viewport
-- itself. Fitting the budget was never the property worth testing for.
--
-- So a named tier now has to earn the viewport twice over. Pass one, finest
-- first, keeps a tier only if it fits AND its largest group is at most half the
-- matches. Pass two, coarsest first, handles the overshoot: take the coarsest
-- tier with more places than the budget and show the largest of them, which
-- turns Dalmatia into its 150 biggest towns and Donji grad into its streets.
-- Truncation is already honest - the existing ORDER BY member_count DESC and
-- LIMIT keep the largest groups, and total_features reports the real count.
--
-- Nothing outside the tier decision changes.
...
BEGIN
  ...
  WITH urgent_institutions AS MATERIALIZED (
    ...
    SELECT count(*)::bigint AS groups, coalesce(max(n), 0)::bigint AS biggest
    FROM (SELECT count(*) AS n FROM candidates GROUP BY key_street) g
  ), district_stat AS (
    SELECT count(*)::bigint AS groups, coalesce(max(n), 0)::bigint AS biggest
    FROM (SELECT count(*) AS n FROM candidates GROUP BY key_district) g
  ), city_stat AS (
    SELECT count(*)::bigint AS groups, coalesce(max(n), 0)::bigint AS biggest
    FROM (SELECT count(*) AS n FROM candidates GROUP BY key_city) g
  ), county_stat AS (
    SELECT count(*)::bigint AS groups, coalesce(max(n), 0)::bigint AS biggest
    FROM (SELECT count(*) AS n FROM candidates GROUP BY key_county) g
  ), stats AS (
    SELECT
      (SELECT count(*) FROM candidates)::bigint AS matches,
      street.groups AS street_groups, street.biggest AS street_biggest,
      district.groups AS district_groups, district.biggest AS district_biggest,
      city.groups AS city_groups, city.biggest AS city_biggest,
      county.groups AS county_groups, county.biggest AS county_biggest
    FROM street_stat street, district_stat district, city_stat city, county_stat county
  ), tier AS (
    SELECT CASE
