-- The map's donation-type filter became multi-select: a visitor can ask for
-- organisations accepting any of several kinds of help, not exactly one.
--
-- The filter is enforced here, not in the browser -- the function clusters and
-- applies the feature limit before returning, so post-filtering the response
-- would report truncation and match counts for a wider set than it shows. The
-- scalar p_donation_type could not simply become text[]: changing a parameter's
-- type is a new signature, and CREATE OR REPLACE would leave an ambiguous
-- overload behind. So both functions are dropped and recreated with
-- p_donation_types appended, exactly as 20260821120000 and 20260822160000 did
-- for their own signature changes.
--
-- p_donation_type is kept and unioned with the new array so a release still
-- sending the scalar keeps filtering correctly during a rolling deploy; the
-- predicate widens from "accepts this one" (@>) to "accepts any of these" (&&).
-- The v1 body is otherwise the 20260825120000 body verbatim.

BEGIN;

-- v2 first: it calls v1, so dropping v1 first would fail on the dependency.
DROP FUNCTION IF EXISTS public.map_association_registry_v2(
  double precision, double precision, double precision, double precision,
  integer, text[], text, boolean, boolean, text, integer, boolean, text
);
DROP FUNCTION IF EXISTS public.map_association_registry_v1(
  double precision, double precision, double precision, double precision,
  integer, text[], text, boolean, boolean, text, integer, boolean, text
);

CREATE FUNCTION public.map_association_registry_v1(p_min_lng double precision, p_min_lat double precision, p_max_lng double precision, p_max_lat double precision, p_zoom integer, p_categories text[] DEFAULT ARRAY[]::text[], p_donation_type text DEFAULT NULL::text, p_only_zagreb boolean DEFAULT false, p_only_urgent boolean DEFAULT false, p_query text DEFAULT NULL::text, p_limit integer DEFAULT 150, p_only_onboarded boolean DEFAULT false, p_city text DEFAULT NULL::text, p_donation_types text[] DEFAULT NULL::text[])
...
BEGIN
  ...
  IF coalesce(cardinality(p_donation_types), 0) > 32 THEN RAISE EXCEPTION 'too many donation types'; END IF;
  IF EXISTS (
    SELECT 1 FROM unnest(coalesce(p_donation_types, ARRAY[]::text[])) donation_value
    WHERE length(donation_value) > 64
  ) THEN RAISE EXCEPTION 'donation type input is too long'; END IF;
  ...
  -- One set from both spellings: the scalar p_donation_type is what every
  -- caller sent before the filter became multi-select, and a deployed
  -- release may still be sending it while this function is already live.
  -- array_agg over an empty set yields NULL, which is the "no filter" case.
  selected_donations := (
    SELECT array_agg(DISTINCT donation_value)
    FROM unnest(coalesce(p_donation_types, ARRAY[]::text[]) || ARRAY[p_donation_type]) AS donation_value
    WHERE donation_value IS NOT NULL AND btrim(donation_value) <> ''
  );
  ...
  WITH urgent_institutions AS MATERIALIZED (
    ...
    WHERE state.singleton = true
      ...
      AND (selected_donations IS NULL OR (i.id IS NOT NULL AND i.accepts_donations && selected_donations))
...
CREATE FUNCTION public.map_association_registry_v2(
  ...
  p_donation_types text[] DEFAULT NULL::text[]
)
RETURNS TABLE(
...
AS $function$
  ...
  FROM public.map_association_registry_v1(
    ...
    p_only_onboarded, p_city, p_donation_types
  ) feature
...
REVOKE ALL ON FUNCTION public.map_association_registry_v1(
  double precision, double precision, double precision, double precision,
  integer, text[], text, boolean, boolean, text, integer, boolean, text, text[]
) FROM PUBLIC;
